...
void 
Parser::add_hints(const std::set<uint64_t>& addrs)
{
    CodeRegion * cr = NULL;
    StandardParseData * spd = dynamic_cast<StandardParseData *>(_parse_data);
    if(!spd) {
        parsing_printf("   add_hints is invalid on overlapping regions\n");
        return;
    }
	...
	Function* f = NULL;
	cr = NULL;
	cr = spd->reglookup(cr, addr);

	if (!cr){
	    parsing_printf("   failed region lookup at %lx\n", addr);
	}

	if (!obj().cs()->regionsOverlap())
	    f = factory()._mkfunc(
		    addr, HINT, "", &obj(), cr, obj().cs());
	else
	    f = factory()._mkfunc(
		    addr, HINT, "", &obj(), cr, cr);
	if (f){
	    parsing_printf("[%s] adding hint from eh_frame %lx\n", FILE__, f->addr());
	    add_hint(f);
	} else {
	    parsing_printf("[%s] adding hint from eh_frame %lx\n, failed!", FILE__, f->addr());
	}
    }
}
...
//                boost::lock_guard<Block> src_guard(*edge->src());
                ...
                if (prevInsn.getOperation().getID() == e_syscall) {
                    ...
                    if (!is_nonret && is_plt) {
			auto tmp_conditional_nonret = obj().cs()->conditionalNonReturning(plt_entries[target], func, work->edge()->src());
                        is_nonret |= (obj().cs()->nonReturning(plt_entries[target]));
			is_nonret |= tmp_conditional_nonret;
                        if (is_nonret) {
