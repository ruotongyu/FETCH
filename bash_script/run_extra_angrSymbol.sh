...
for file in `find $DIR -name *.strip | grep -v frame | grep -v O1 | grep -v O0 | grep -v _m32 | grep -v ida_ | grep -v shuffle`; do
  ...
  angr_file=${sdir}/Block-angrSymbol-${base_name}.pb
