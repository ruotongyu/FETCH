...
for file in `find $DIR -name *.strip | grep -v frame | grep -v _strip | grep Os | grep -v _m32 | grep -v ida_ | grep -v shuffle`; do
  ...
  TailCall_file=${dir_name}/TailCallWithoutRef_${base_name}.pb
  ...
  timeout 1h python3 $TOOL -g $gtBlock_file -i $TailCall_file -b $binary_file -e $ehname -r $ref
done
