...
for file in `find $DIR -name *.strip | grep -v frame | grep -v _strip | grep -v O0 | grep -v ccr_m32 | grep -v ida_ | grep -v shuffle`; do
  ...
  angr=${sdir}/Block-angrBB_Stack-$base_name.pb
  ...
  echo "<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>"
  echo "<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>"
  echo "python3 $TOOL -e $ehfile -a $angr -t angr"
  python3 $TOOL -e $ehfile -a $angr -t angr
done
