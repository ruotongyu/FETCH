...
for file in `find $DIR -name *.strip | grep -v frame | grep -v _strip | grep -v O0 | grep -v O1 | grep -v _m32 | grep -v ida_ | grep -v shuffle`; do
  ...
  ghidra_file=${sdir}/Block-ghidraNoFuncMatching-${base_name}.strip.pb
  ...
  #echo "timeout 1h python3 $TOOL -g $gtBlock_file -c $ghidra_file -b $binary_file"
  timeout 1h python3 $TOOL -g $gtBlock_file -c $ghidra_file -b $binary_file
done
