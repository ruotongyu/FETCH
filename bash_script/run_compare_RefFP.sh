...
for file in `find $DIR -name *.strip | grep -v frame | grep -v _strip | grep O0 | grep -v _m32 | grep -v ida_ | grep -v shuffle`; do
  ...
  timeout 1h python3 $TOOL -g $gtBlock_file -i $ehRes_file -e $ehname -b $binary_file -r $ref
done
